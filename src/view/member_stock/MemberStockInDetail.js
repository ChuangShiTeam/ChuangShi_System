...
class MemberStockInDetail extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		notification.on('notification_member_stock_in_detail_view', this, function (data) {
			this.setState({
				is_show: true,
	...
	componentWillUnmount() {
		notification.remove('notification_member_stock_in_detail_view', this);
	}
	...
	render() {
		...
		return (
			<Modal title={'会员入库单明细'} maskClosable={false} width={document.documentElement.clientWidth - 200} className="modal"
...
MemberStockInDetail.propTypes = {};

MemberStockInDetail = Form.create({})(MemberStockInDetail);

export default connect(({member_stock_in}) => ({member_stock_in}))(MemberStockInDetail);