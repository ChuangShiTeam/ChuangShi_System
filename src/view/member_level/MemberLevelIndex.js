...
class MemberLevelIndex extends Component {
    ...
    componentDidMount() {
        if (constant.action === 'system') {
            ...
            app_id: this.props.member_level.app_id
            });
    ...
    handleLoad() {
        ...
        http.request({
            url: '/member/level/' + constant.action + '/list',
            data: {
                app_id: this.props.member_level.app_id,
                member_level_name: this.props.member_level.member_level_name,
    ...
    handleAdd() {
        notification.emit('notification_member_level_detail_add', {});
    }

    handleEdit(member_level_id) {
        notification.emit('notification_member_level_detail_edit', {
            member_level_id: member_level_id
        });
    }

    handleDel(member_level_id, system_version) {
        this.setState({
            is_load: true
        });

        http.request({
            url: '/member/level/' + constant.action + '/delete',
            data: {
                member_level_id: member_level_id,
    ...
    render() {
        ...
        const columns = [{
            ...
            render: (text, record, index) => (
                <span>
                  <a onClick={this.handleEdit.bind(this, record.member_level_id)}>{constant.edit}</a>
                  <span className="divider"/>
                  <Popconfirm title={constant.popconfirm_title} okText={constant.popconfirm_ok}
                              cancelText={constant.popconfirm_cancel}
                              onConfirm={this.handleDel.bind(this, record.member_level_id, record.system_version)}>
                    <a>{constant.del}</a>
                  </Popconfirm>
                </span>
            )
