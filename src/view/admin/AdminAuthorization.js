...
class AdminAuthorization extends Component {
	...
	componentDidMount() {
		notification.on('notification_admin_authorization', this, function (data) {
			this.setState({
				is_show: true,
	...
	componentWillUnmount() {
		notification.remove('notification_admin_authorization', this);
	}
	...
	render() {
		...
		return (
			<Modal title={'管理员授权角色'} maskClosable={false} width={document.documentElement.clientWidth - 200}
				   className="modal"
...
export default connect(({admin}) => ({admin}))(AdminAuthorization);